...
function displayParts() {
  ...
  for (var i = 0; i < mecha.partList.length; i++) {
    partsString +=
      '<div data-id="' + i + '" class="">' + mecha.partList[i].type;
    switch (mecha.partList[i].type) {
      case "Weapon Link":
        partsString +=
          '<input class="partVal1" data-index="' +
          i +
          '" value="' +
          mecha.partList[i].val1 +
          '"/>';
        partsString +=
          '<input class="partCP" data-index="' +
          i +
          '" value="' +
          mecha.partList[i].CP +
          ' CP"/>';
...
function initPartsUI() {
  ...
  for (var i = 0; i < partList.length; i++) {
    $("#partSelect").append(
      '<div class="partBtn" data-part-id="' + i + '">' + partList[i] + "</div>"
    );
  }
  //PART SELECTION
