...
//in vec2 uv;
in vec2 oUv;
//out vec4 fragColor;//use gl_FragColor instead of fragColor
...
#define MOON_ROTATION_AXIS (MOON_OFFSET - uPlanetPosition) * rotateZ(myPI/2.)
...
vec3 atmosphereColor(vec3 ro, vec3 rd, float spaceMask, Hit firstHit) {
  ...
  float moonMask = (1.0 - spaceMask) * step(uPlanetRadius + myEPSILON, length(position - uPlanetPosition));
...
vec3 radiance(vec3 ro, vec3 rd) {
  ...
  if(hit.len < INFINITY) {
    ...
    Hit shadowHit = intersectScene(hitPosition + myEPSILON * uSunDirection, uSunDirection);
