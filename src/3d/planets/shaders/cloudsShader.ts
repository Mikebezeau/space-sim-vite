...
varying:
    vUv // included in ShaderMaterial code
*/
...
vec2 cloudUv = vUv * 2.0 - 1.0;
float theta = cloudUv.x * 3.14159265359;
float phi = cloudUv.y * 1.57079632679;
...
float cloudMask = fractalNoise(distortedP * (1.0 / cloudScale) + vec3(u_time * 0.05));
...
//float shadowFactor = clamp(dot(lightDir, vCloudNormalView), 0.0, 1.0);
// using the rotatedNormal 
...
vec4 cloudColor = mix(gl_FragColor, baseCloudColor, cloudMask);
