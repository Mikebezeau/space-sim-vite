...
interface MechBPInt {
  ...
  crewSP: () => number;
  crewCP: () => number;
  getServoById: (servoId: string) => MechServo | undefined;
  servoWeaponList: (servoId: string) => any[];
  findWeaponId: (weaponId: string) => any;
  totalWeight: () => number;
...
class MechBP implements MechBPInt {
  id: string;
  name: string;
  scale: number;
  ...
  getServoById(servoId: string) {
    return this.servoList.find((s) => s.id === servoId);
  }
  ...
  findWeaponId(weaponId: string) {
    return this.weaponList.find((w) => w.id === weaponId);
  }
