...
export const period = (separation, smallMass, largeMass) => {
  return (
    Math.sqrt(Math.pow(separation, 3) / (smallMass + largeMass)) *
    C.DAYS_IN_A_YEAR
  );
};
...
export const dayLength = (planet) => {
  ...
  temp = 1.0 / ((base_angular_velocity / 2) * Math.PI * C.SECONDS_PER_HOUR);
...
export const molecule_limit = (planet) => {
  return (
    (3.0 * C.MOLAR_GAS_CONST * planet.exosphericTemperature) /
    Math.pow(
      planet.escapeVelocity / C.GAS_RETENTION_THRESHOLD / C.CM_PER_METER,
      2.0
    )
  );
};
...
export const acceleration = (mass, radius) => {
  return (
    (C.GRAV_CONSTANT * (mass * C.SOLAR_MASS_IN_GRAMS)) /
    Math.pow(radius * C.CM_PER_KM, 2)
  );
};
...
export const boiling_point = (surf_pressure) => {
  const surface_pressure_in_atmospheres =
    surf_pressure / C.EARTH_SURF_PRES_IN_MILLIBARS;
  return (
    (373.15 * (5.78 - 0.15 * Math.log(surface_pressure_in_atmospheres))) /
...
export const ice_fraction = (hydro_fraction, surf_temp) => {
  ...
  if (surf_temp > 328) surf_temp = 328;
...
export const green_rise = (optical_depth, effective_temp, surf_pressure) => {
  ...
  return (
    (Math.pow(1.75 * optical_depth, 1 / 4) - 1.0) *
    effective_temp *
    convection_factor
  );
};
...
export const calculate_surface_temp = (
  ...
  if (!first) {
    ...
    planet._surfaceTemperature =
      (planet.surfaceTemperature + last_temp * 2) / 3;
  }
...
export const iterate_surface_temp = (planet) => {
  ...
  for (let count = 0; count <= 25; count++) {
    ...
    if (Math.abs(planet.surfaceTemperature - last_temp) < 0.25) break;
  }
...
export const iterate_surface_temp2 = (planet) => {
  ...
  while (Math.abs(surf1_temp - previous_temp) > 1.0 && iterations++ < 1000) {
    ...
    if (
      surf1_temp >= planet.boilingPoint ||
      surf1_temp <= C.FREEZING_POINT_OF_WATER
    )
      water = 0.0;
...
export const get_temp_range = (planet) => {
  ...
  var max =
    planet.surfaceTemperature + Math.sqrt(planet.surfaceTemperature) * 10;
...
export function calculate_gases(sun, planet /*, planet_id*/) {
  ...
  for (let i = 0; i < gases.length; i++) {
    ...
    if (
      yp >= 0 &&
      yp < planet.temperature.min &&
      gases[i].weight >= planet.molecularWeight
    ) {
      ...
      if (gases[i].sym === "Ar") {
        ...
        pres2 = 0.75 + pressure;
        react = Math.pow(
          1 / (1 + gases[i].reactivity),
          (sun.age / 2e9) * pres2
        );
      } else if (
        (gases[i].sym === "O" || gases[i].sym === "O2") &&
        sun.age > 2e9 &&
        planet.surfaceTemperature > 270 &&
        planet.surfaceTemperature < 400
      ) {
  ...
  for (var i = 0; i < planet.atmosphere.length; i++) {
    if (planet.atmosphere[i].amount > 0.0) {
      ...
      planet.atmosphere[n].surfacePressure =
        (planet.surfaceTemperature * planet.atmosphere[i].amount) / totamount;
      n++;
    }
  }
