...
import HudTargetController, {
  MAX_COMBAT_TARGETS,
} from "../classes/hudTargets/HudTargetController";
...
interface hudTargetingStoreState {
  // CSS HUD targets
  ...
  // selected and focused target ids
...
const useHudTargtingStore = create<hudTargetingStoreState>()((set, get) => ({
    ...
    if (
      ...
      useStore.getState().player.mechBP.weaponList.forEach((weapon) => {
        ...
        if (weaponHud) {
          ...
          if (weapon.getAmmoCount() !== null) {
            weaponHud.ammoCount =
              weapon.getAmmoCount()! -
              (useStore.getState().player.ammoUsed[weapon.id]?.ammoUsed || 0); // get ammo count from weapon, subtract used ammo
            ...
            weaponHud.ammoCount = null; // no ammo count for non-projectile weapons
          }
    ...
    if (
      ...
      set({
        scanProgressNormHudTarget:
          get().hudTargetController.getSelectedHudTarget()?.scanProgressNorm ||
          0,
      });
    }
  },
      ...
      const incrementScanProgress = () => {
        ...
        if (get().isShowScanButton) {
          if (selectedTarget.scanProgressNorm < 1) {
            ...
            get().scanTargetTimeoutId = setTimeout(incrementScanProgress, 200);
          } else {
            selectedTarget.scanProgressNorm = 1;
            get().scanTargetTimeoutId = null;
          }
