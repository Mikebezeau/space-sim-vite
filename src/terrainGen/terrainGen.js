...
export default class Terrain {
  constructor(seed, genCities, hills = 1, elevation = 0) {
    ...
    this._simplex = SimplexNoise(seed);
  ...
  _checkZoneFlattness(i, j, citySizeVerticesHalf, averageHeight) {
    ...
    for (let k = i - citySizeVerticesHalf; k <= i + citySizeVerticesHalf; k++) {
      for (
        ...
        const fVal = sNoise(ni, nj) + 1;
        const sVal = 0.5 * sNoise(ni * 2, nj * 2) + 1;
        const tVal = 0.25 * sNoise(ni * 4, nj * 4) + 1;
      ...
      cities.forEach((city, index) => {
        if (index !== 0) {
          ...
          */
          ...
          //g.setAttribute( 'normal', new THREE.BufferAttribute( g.normals, 3 ).setDynamic( true ) );
