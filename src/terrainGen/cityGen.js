...
function getNoiseValue(x, y, frequency) {
  return Math.abs(sNoise(x / frequency, y / frequency));
}
...
function generateGroundBlocks(rng, gridSize /*, elevation*/) {
  ...
  if (gridSize > 8) {
    ...
    pyramid.position.set(0, (blockSize * gridSize) / 20, 0);
    ...
    for (let j = 0; j < gridSize; j++) {
      ...
      if (isGroundBlock(i, j)) {
        ...
        if (isBuildingBlock(i, j)) {
          ...
          let buildingWidth = curbWidth - blockMargin * 2;
